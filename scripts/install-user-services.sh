...
cp "${ROOT_DIR}/deploy/systemd/gpu-factory-mcp.service" "${UNIT_DIR}/"

echo "Installed user units to ${UNIT_DIR}"
echo "Next run in a real host shell:"
echo "  systemctl --user daemon-reload"
echo "  systemctl --user enable --now gpu-factory-api.service gpu-factory-worker.service gpu-factory-mcp.service"
